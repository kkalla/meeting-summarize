...
[![CI](https://github.com/kkalla/meeting-summarize/actions/workflows/ci.yml/badge.svg)](https://github.com/kkalla/meeting-summarize/actions/workflows/ci.yml)
...
[![Lint: flake8](https://img.shields.io/badge/lint-flake8-4B8BBE)](https://flake8.pycqa.org/)
...
## 4. 자동화: 폴더 watcher (launchd)

수동 실행 대신, **watcher 를 상시 띄워두고 폴더에 녹음만 떨구면** 자동으로 요약되게 할 수 있다.
...
macOS 에서는 호스트 whisper.cpp(Metal 가속, RTF~0.6)를 그대로 쓰도록 **launchd LaunchAgent** 로
실행한다. 도커 CPU 빌드보다 전사가 대략 5배 빠르다.

사전 준비: ① `./setup.sh` 로 whisper.cpp(Metal) 빌드 + 모델(.bin)을 받아두고, ② `.env` 에
`OPENROUTER_API_KEY` 를 채운다.

```bash
./deploy/install-launchd.sh        # LaunchAgent 등록 + 자동 시작(로그인 시·죽으면 자동 재시작)
tail -f logs/watcher.log           # 처리 로그 확인
cp 회의녹음.m4a data/inbox/         # 떨구면 수 초 내 자동 처리

# 중지/해제
launchctl unload -w ~/Library/LaunchAgents/com.max.meeting-watcher.plist
```

watcher 설정(폴링 주기·안정화 횟수·대상 확장자·폴더 경로)은 `configs/pipeline.yaml` 의 `watcher` 섹션에서 조정한다.
데몬 없이 한 번만 돌리려면 `uv run python scripts/run_watcher.py` 도 가능하다.

> **전사 ETA 자동 보정**: 시작 로그의 "예상 ~N초" 는 직전 전사들의 실측 RTF 를 EMA 로 누적
> (`data/transcripts/.rtf_state`)해 자동 갱신된다. 하드웨어(Metal/CPU)가 바뀌어도 첫 전사
> 이후부터 실제 소요시간에 수렴하므로 `stt.rtf_estimate` 는 콜드 스타트 초기값으로만 쓰인다.

### (대안) 도커 컨테이너

서버 등 macOS 가 아닌 환경에서는 `compose.yaml`/`Containerfile` 로 컨테이너(whisper.cpp CPU 빌드)를
띄울 수도 있다. 이 경우 `configs/pipeline.yaml` 의 경로를 컨테이너 절대경로(`/data/...`)로 되돌려야 한다.

```bash
podman compose up -d --build      # 최초 빌드 후 백그라운드 실행
podman compose logs -f watcher    # 처리 로그 확인
podman compose down               # 중지(graceful 종료)
```
